...
(function ($) {
  ...
  <div class="theme-demos">
    <div class="demo-button-wrapper">
      ...
      <div class="all-demo">
        <a href="#">
          <div class="theme-wrapper"><div>FTPL</div></div>
        </a>
      </div>
    </div>
    <span class="close-popup fs-22 text-dark-gray w-40px h-40px d-flex justify-content-center align-items-center">
      <i class="fa-solid fa-xmark"></i>
    </span>
  </div>
`;

// Append the shell to the body
$('body:not(.landing-page)').append(themeDemoHTML);

// Handle click event to load large HTML
$(document).on('click', '.all-demo', function (e) {
  e.preventDefault();
  var themeDemosObj = $(this).parents('.theme-demos');

  if (!themeDemosObj.hasClass('active')) {
    $.get('/theme-demos.html', function (data) {
      themeDemosObj.append(data); // inject the HTML file content
      themeDemosObj.addClass('active');
      $('body').addClass('overflow-hidden');
    });
  } else {
    themeDemosObj.removeClass('active');
    themeDemosObj.find('.theme-demo-popup').remove(); // cleanup if needed
